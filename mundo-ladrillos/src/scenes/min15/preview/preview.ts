...
import { BALCONY_BOUNDS } from '../props/balcony';

/**
 * PREVIEW jugable del tramo MINUTO 15–20 · "EL CORDÓN ROJO Y LOS SHOFAROT"
 * (escenas 26–33). App autocontenida para PROBAR e ITERAR: selector de escena,
 * jugador con colisiones, sonido procedural (ambiente + SFX), HUD (objetivo +
 * barras de progreso/alarma), interacción E/botón, mandos táctiles y secuencia
 * encadenada. Mismos HOOKS DE QA que 0–5/5–10/10–15 para el niño sintético "Eli".
 *
 * AUDIO: el clip real del min 15–20 es privado y aún no está en el repo, así que
 * el "film-spine" está APAGADO (USE_FILM_SPINE=false). Suenan ambiente + SFX +
 * subtítulos; cuando el usuario pase el audio, se enciende como en min05.
 */
...
// --- Luces (se reconfiguran por 'mundo') ---
const hemi = new THREE.HemisphereLight(0x4c6690, 0x2a2a2a, 1.0); scene.add(hemi);
const key = new THREE.DirectionalLight(0xc2d4f7, 1.3);
key.position.set(-120, 140, 200); key.castShadow = true;
key.shadow.mapSize.set(2048, 2048);
key.shadow.camera.near = 1; key.shadow.camera.far = 320;
key.shadow.camera.left = -60; key.shadow.camera.right = 60;
key.shadow.camera.top = 70; key.shadow.camera.bottom = -60;
...
function applyLighting(mundo: Min15Scene['mundo']): void {
  ...
  if (mundo === 'campamento' || mundo === 'taller') {
    ...
    // balcón / monte: noche azul con luna
...
const progBar = (() => {
  const wrap = mkDiv({ left: '50%', top: '116px', transform: 'translateX(-50%)', width: '240px', height: '15px', background: 'rgba(0,0,0,.45)', borderRadius: '8px', border: '1px solid rgba(255,255,255,.25)', display: 'none', overflow: 'hidden' });
  const fill = document.createElement('div');
  Object.assign(fill.style, { position: 'absolute', left: '0', top: '0', height: '100%', width: '0%', background: 'linear-gradient(90deg,#8fe0ff,#bfffce)', transition: 'width .08s linear' } as CSSStyleDeclaration);
  wrap.appendChild(fill); return { wrap, fill };
})();
...
let currentDef: Min15Scene | null = null;
...
let started = false;
let pendingIntro: Min15Scene['intro'] | null = null;
...
function loadScene(i: number): void {
  const idx = ((i % MIN15_SCENES.length) + MIN15_SCENES.length) % MIN15_SCENES.length;
  const def = MIN15_SCENES[idx];
  ...
  const amb = def.ambiente ?? 'night';
  if (sound.ready) sound.setAmbience(amb === 'day' ? 'day' : amb === 'river' ? 'river' : amb === 'street' ? 'street' : 'night');
  ...
  sceneTag = mountSceneTag({
    id: `E${def.numero}`, nombre: def.titulo, tramo: 'T15–20', modo: def.mundo, hilo: 'min15',
    archivo: `${def.id}.ts`,
  ...
  // límites del recinto POR ESCENA (cerrar laterales: el niño no rodea la escena)
  if (def.mundo === 'balcon') controller.setBounds(BALCONY_BOUNDS.minX, BALCONY_BOUNDS.maxX, BALCONY_BOUNDS.minZ, BALCONY_BOUNDS.maxZ);
  else controller.setBounds(-26, 26, def.spawn.z - 8, (def.objetivo.target?.z ?? def.spawn.z) + 14);
...
(window as any).__loadNumero = (n: number) => { const idx = MIN15_SCENES.findIndex((s) => s.numero === n); if (idx >= 0) loadScene(idx); };

// ================= Pantalla de inicio =================
const startEl = document.createElement('div');
startEl.innerHTML =
  '<div style="text-align:center;color:#f4e9d2;font-family:system-ui,sans-serif;padding:24px;max-width:540px">' +
  '<div style="font:800 30px/1.1 Georgia,serif;color:#e8b04b;letter-spacing:1px">EL CORDÓN ROJO</div>' +
  '<div style="opacity:.85;margin:12px 0 22px">Minuto 15–20 · mundo de ladrillo. Ata el cordón rojo en la ventana de Rahab y descuélgate por la muralla para escapar de Jericó.</div>' +
...
Object.assign(startEl.style, { position: 'fixed', inset: '0', zIndex: '50', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(120% 100% at 50% 0%, #1a2740, #060b12 72%)', transition: 'opacity .4s' } as CSSStyleDeclaration);
document.body.appendChild(startEl);
const startGame = (): void => {
  sound.init();
  if (currentDef) sound.setAmbience('night');
...
const startIdx = Number.isFinite(startNum) ? MIN15_SCENES.findIndex((s) => s.numero === startNum) : 0;
...
function animate(now: number): void {
  ...
  if (current && currentDef) {
    ...
    updateConfetti(dt);
    if (done) { advanceT += dt; if (advanceT > 2.8) { const idx = MIN15_SCENES.findIndex((s) => s.id === currentDef!.id); loadScene(idx + 1); } }
  }
...
// Mismo contrato que 0–5 / 5–10 / 10–15 para que su arnés juegue este tramo
// SIN adaptaciones:  __loadNumero(n) · __walk(x,z,step)→bool · __act() · __probe()
